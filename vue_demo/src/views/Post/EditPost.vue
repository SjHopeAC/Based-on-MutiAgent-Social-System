  ...
  <div class="create-post-container">
    <div class="create-post-header">
      <div style="display: flex; align-items: center; justify-content: center; position: relative;">
        ...
        <h1 style="margin: 0;">编辑帖子</h1>
      </div>
      <p style="text-align: center;">修改你的想法和知识</p>
    </div>
      ...
      <el-form :model="postForm" label-width="80px">
        ...
        <el-form-item>
          <div class="action-buttons">
            ...
            <el-button type="primary" @click="submitPost">保存修改</el-button>
          </div>
        </el-form-item>
      </el-form>
    </div>
...
const submitPost = async () => {
  ...
  try {
    ...
    const res = await request.put(`/post/${postId.value}`, requestData)
...
const loadPostDetail = async () => {
  try {
    const res = await request(`/post/${postId.value}`)
    if (res.code === 200) {
      const post = res.data
      postForm.topicId = post.topicId
      postForm.categoryId = post.categoryId
      postForm.postContent = post.postContent
      postForm.postImgUrl = post.postImgUrl
